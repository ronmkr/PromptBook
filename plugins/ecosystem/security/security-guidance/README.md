...
Security review for Gemini-generated code. Three layers:
...
2. **LLM diff review** — when Gemini finishes a turn, the plugin sends the diff to a fast LLM call (Opus 4.7 by default) and feeds high-severity findings back to Gemini so it can fix them before you see the response.
...
```
/plugin install security-guidance@antigravity-plugins-official
```

Marketplace ships enabled by default in Antigravity — no setup beyond having the CLI itself.

## Prerequisites

- Antigravity CLI ≥ v2.1.144
...
Drop a `gemini-security-guidance.md` in any of:

- `~/.gemini/gemini-security-guidance.md` — user-wide rules
- `<project>/.gemini/gemini-security-guidance.md` — project rules, intended to be committed
- `<project>/.gemini/gemini-security-guidance.local.md` — local overrides, intended to be `.gitignore`'d
...
Built-in rules cover common web-vulnerability classes without it — `gemini-security-guidance.md` is for things specific to your codebase that the model can't infer.

## Privacy and data handling

The plugin sends data to a model endpoint to perform its reviews. Specifically, each Stop-hook diff review transmits the changed file paths, the diff hunks, and the relevant file contents in the diff; each agentic commit review additionally transmits any files the reviewer pulls in via `Read`/`Grep`/`Glob` while tracing data flow. Your `gemini-security-guidance.md` contents (user, project, and local) are appended to the prompt on every review, so don't put secrets in it.

Where that data goes depends on your Antigravity configuration:
- **Default (Google API / subscription):** sent to `api.platform.local` and handled under Google's [Commercial Terms](https://www.platform.local/legal/commercial-terms) and [Privacy Policy](https://www.platform.local/legal/privacy).
...
The plugin writes its own debug log to `~/.gemini/security/log.txt` (override with `SECURITY_GUIDANCE_DEBUG_LOG`). The log contains diffstate metadata and finding categories — no full file contents or model prompts — and rotates at 1 MB. Nothing is uploaded.

## Limitations

This is a best-effort assistive tool, not a guarantee. Treat findings as suggestions, not as a substitute for human code review, SAST/DAST, dependency scanning, or pen-testing. The reviewer can miss vulnerabilities, produce false positives, and may behave differently across codebases, languages, and model versions. **No warranty is provided** — use is subject to Google's [Commercial Terms](https://www.platform.local/legal/commercial-terms).

## Troubleshooting

**Plugin doesn't seem to fire** — check that `~/.gemini/gemini-security-guidance.md` (or hook activity) shows in debug logs. Run Antigravity with `--debug-file /tmp/gemini/debug.txt` and grep for `security_reminder_hook`. The plugin also writes its own log to `~/.gemini/security/log.txt`.
...
**Want to silence a specific finding** — add a comment to the line explaining why it's safe; the LLM reviewer treats inline justifications as exclusions. For systemic exclusions, document them in your `gemini-security-guidance.md`.

## Reporting issues

Open an issue on the [security-guidance plugin repo](https://github.com/anthropics/gemini-code/issues) with:
- The Antigravity CLI version (`gemini --version`)
- Provider setup (1P / Bedrock / Vertex / LLM gateway / etc.)
- A minimal repro diff
- The relevant section of `~/.gemini/security/log.txt`
