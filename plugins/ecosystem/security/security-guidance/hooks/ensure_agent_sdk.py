#!/usr/bin/env python3
"""SessionStart bootstrap: ensure gemini_agent_sdk is importable for the
agentic commit reviewer.

If gemini_agent_sdk already imports in the current python3, this is a no-op.
Otherwise it creates a venv at ~/.gemini/security/agent-sdk-venv and installs
...
The venv lives under ~/.gemini/security/ (same dir the plugin already uses
...
# Shared state-dir resolver: SECURITY_WARNINGS_STATE_DIR → GEMINI_CONFIG_DIR/security
# → ~/.gemini/security. See _base.state_dir for resolution precedence. Re-aliased
...
NOOP_SYSTEM = 0      # gemini_agent_sdk already importable in system python
...
def _sdk_on_syspath() -> bool:
    ...
    try:
        return importlib.util.find_spec("gemini_agent_sdk") is not None
    except Exception:
        return False
...
def _plugin_version_int() -> int:
    ...
    try:
        p = Path(__file__).parent.parent / ".antigravity-plugin" / "plugin.json"
...
def main() -> tuple[int, str, str]:
    ...
    if venv_py.exists():
        try:
            r = subprocess.run(
                [str(venv_py), "-c", "import gemini_agent_sdk"],
...
def _maybe_emit_user_notice(outcome: int, pv: int) -> str | None:
    ...
    return (
        ...
        f"(e.g. `brew install python` on macOS) and restart Antigravity.\n\n"
        f"This notice is shown once per plugin version. "
        f"See: github.com/gemini-cli-extensions/antigravity-plugins-official/issues/2071"
    )
