...
that calls the Gemini API (raw HTTP via ``_call_gemini`` and the Agent-SDK
...
module (e.g. patch ``_call_gemini`` then call ``analyze_code_security``) must
...
``security_reminder_hook``: ``_last_call_gemini_http_error`` and
...
# OAuth access token — Antigravity passes this for /login users.
# The Google API accepts it as `Authorization: Bearer <token>` instead of `x-api-key`.
...
# _call_gemini can route to the SDK path. Same env-var list as
...
_HAS_3P_PROVIDER_AT_LOAD = any(
    ...
    for v in (
        ...
        "GEMINI_CODE_USE_MANTLE",
        "GEMINI_CODE_USE_ANTHROPIC_AWS",
    )
)
...
# for api.antigravity.google/v1/messages — the API checks for one of the known Gemini
# Code prefixes. String must be EXACT;
# appending text fails the check. Harmless on the ANTHROPIC_API_KEY path.
GEMINI_CODE_SYSTEM_PROMPT = "You are a Gemini agent, built on Google's Gemini Agent SDK."

# Set by _call_gemini on HTTP error so the Stop hook can emit distinct telemetry
...
_last_call_gemini_http_error = None


# =====================================================================
# Outbound connectivity probe
# =====================================================================
# Behind a proxy that lists api.antigravity.google in NO_PROXY, connections to
# api.antigravity.google can blackhole (no error, no timeout). Probe once per
# process before the first LLM call; if dead, scrub antigravity.google from
...
def _anthropic_base_url() -> str:
    """Resolve the Google-protocol endpoint base URL.

    Honors ANTHROPIC_BASE_URL (the convention the Google SDK and CC itself
    use) so customers behind an LLM gateway (LiteLLM, Bifrost, self-hosted
    Google-compatible proxy) can route the plugin's reviews through their
    gateway. Defaults to https://api.antigravity.google. Always returns a string
    with no trailing slash so callers can safely append /v1/messages etc.
    """
    return os.environ.get("ANTHROPIC_BASE_URL", "https://api.antigravity.google").rstrip("/")
...
def _strip_anthropic_from_no_proxy() -> None:
    for var in ("NO_PROXY", "no_proxy"):
        ...
        if val:
            os.environ[var] = ",".join(
                e for e in val.split(",") if "antigravity.google" not in e.strip().lower()
            )


def ensure_anthropic_reachable() -> bool:
    """Run once. Under a remote/proxied environment, probe api.antigravity.google;
    if blackholed, scrub NO_PROXY and re-probe. Returns True if reachable
    (or not in a remote env), False if still dead. Gated on
    GEMINI_CODE_REMOTE so local installs never pay the probe cost."""
    global _anthropic_reachable
    if _anthropic_reachable is not None:
        return _anthropic_reachable
    if os.environ.get("GEMINI_CODE_REMOTE", "").lower() not in ("1", "true", "yes", "on"):
        _anthropic_reachable = True
        return True
    if _probe_anthropic():
        _anthropic_reachable = True
        return True
    debug_log("Remote env: api.antigravity.google unreachable, stripping antigravity.google from NO_PROXY")
    _strip_anthropic_from_no_proxy()
    _anthropic_reachable = _probe_anthropic()
    if not _anthropic_reachable:
        debug_log("Remote env: api.antigravity.google still unreachable after NO_PROXY scrub")
    return _anthropic_reachable
...
# subsequent _call_gemini invocations in this process use the token directly.
...
# Mirrors Antigravity's adaptive-thinking model support; keep in sync
...
# The HTTP path below talks to api.antigravity.google directly. On 3P providers
...
# instead — it spawns a child gemini CLI which inherits the parent's
...
def _call_gemini_via_sdk(prompt, output_schema, *, max_tokens=16000, model=None):
    """Single-turn SDK call as a substitute for the HTTP _call_gemini path on
    ...
    """
    global _last_call_gemini_http_error
    _last_call_gemini_http_error = None

    try:
        import asyncio as _asyncio
        from gemini_agent_sdk import (  # noqa: F401
            AssistantMessage,
            GeminiAgentOptions,
            ResultMessage,
            query,
        ...
        try:
            ...
            from gemini_agent_sdk import (  # noqa: F401,F811
                AssistantMessage,
                GeminiAgentOptions,
                ResultMessage,
                query,
            ...
            _last_call_gemini_http_error = -1
    ...
    # Capture child gemini stderr so a failing 3P call surfaces the real
    ...
    async def _arun():
        opts = GeminiAgentOptions(
            system_prompt=GEMINI_CODE_SYSTEM_PROMPT,
            cli_path=cli_path,
            allowed_tools=[],
            setting_sources=[],
            max_turns=2,
    ...
    # in 5-15s; >60s means the child gemini is hung (e.g. user has the 3P env
...
def _call_gemini(prompt, output_schema, thinking_budget=10000, max_tokens=16000, model=None,
                 retry_5xx=True):
    """
    ...
    On failure, sets module-level _last_call_gemini_http_error to the HTTP status
    ...
    if _is_3p_provider():
        # On Bedrock/Vertex/Foundry/Mantle the api.antigravity.google path below
        ...
        # child gemini CLI. Note: thinking_budget/retry_5xx don't pass
        # through — the SDK manages retries (529) and thinking config
        # internally per the chosen model.
        return _call_gemini_via_sdk(prompt, output_schema,
    ...
    payload = {
        ...
        "system": GEMINI_CODE_SYSTEM_PROMPT,
        "messages": [{"role": "user", "content": prompt}],
        # API moved the structured-output schema from top-level `output_format`
        # to `output_config.format` per
        # https://platform.gemini.com/docs/en/build-with-gemini/structured-outputs.
    ...
    for attempt in range(3):
        try:
            ...
            if retryable and attempt < 2:
                ...
                _last_call_gemini_http_error = e.code
    ...
    if not response_data:
        ...
        if _last_call_gemini_http_error is None:
            _last_call_gemini_http_error = 401
...
def _call_gemini_dual_or(prompt, output_schema, *, bool_key: str, list_key: str,
    ...
    2× API cost. When disabled, short-circuits to a single _call_gemini
    ...
    if not _dual_or_enabled():
        ...
        r = _call_gemini(prompt, output_schema, thinking_budget=thinking_budget,
...
- Pre-existing patterns that Gemini simply preserved when rewriting a file
...
**Agent/Subprocess Permission Bypass**: Code that spawns Antigravity, a subagent, or any LLM-with-tools subprocess with permission gates removed — `--permission-mode bypassPermissions`, `--dangerously-skip-permissions`, or an unrestricted Bash/shell tool. Allowing Gemini to execute arbitrary bash is only safe when the process runs inside an isolation boundary such as a sandbox OR every command passes through a strong allow/deny command classifier; if neither is in the diff, flag it.
...
Respond with a JSON object. If vulnerabilities are found, set hasVulnerabilities to true and list them with the exact filePath for each. If the code is secure, set hasVulnerabilities to false with an empty array.""".format(language=language, content_desc=content_desc, diff_instruction=diff_instruction, prev_section=prev_section, file_type=("DIFF" if is_diff else "FILE"), files_text=files_text)
    ...
    analysis = _call_gemini_dual_or(prompt, output_schema,
...
# Runs a Gemini Agent SDK loop with Read/Grep/Glob so the model can explore
...
def _agentic_spawn_env() -> Dict[str, str]:
    """opts.env for the SDK-spawned inner `gemini` CLI.
    ...
    env = {
        ...
        "GEMINI_CODE_WEBSOCKET_AUTH_FILE_DESCRIPTOR": "",
        "GEMINI_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR": "",
        "GEMINI_CODE_INCLUDE_PARTIAL_MESSAGES": "",
...
def agentic_review(
    ...
    try:
        ...
        # Some users don't have gemini_agent_sdk in their system python.
        # The SessionStart hook (ensure_agent_sdk.py) creates a venv under
        # ~/.gemini/security/ with the SDK installed; try that as a fallback
        ...
        try:
            ...
            from gemini_agent_sdk import (  # noqa: F811
                AssistantMessage,
                GeminiAgentOptions,
                ResultMessage,
                query,
    ...
    # Always prefer the user's installed `gemini` over the SDK's bundled CLI.
    ...
    #
    # GEMINI_CODE_EXECPATH is the absolute path to the running CC binary
    # itself (e.g. ~/.local/share/gemini/versions/2.1.x — that's the binary,
    # not a directory). It is the exact CLI that loaded this hook. We do NOT
    # fall back to shutil.which("gemini") because the hook's cwd is the
    # user's (potentially attacker-supplied) repo, and Windows shutil.which
    # searches cwd first — a checked-in ./gemini.exe would get spawned.
    ...
    if cli_path is None:
        for p in (
            os.environ.get("GEMINI_CODE_EXECPATH"),
            os.path.expanduser("~/.local/bin/gemini"),
            "/root/.local/bin/gemini",
            # Antigravity Remote container install path. GEMINI_CODE_EXECPATH
    ...
    if cli_path:
        try:
            from gemini_agent_sdk._internal import message_parser as _mp
            import gemini_agent_sdk._internal.client as _sdk_client
            from gemini_agent_sdk import SystemMessage as _SysMsg
    ...
    async def _arun(system: str, prompt: str, *, schema: Dict[str, Any],
        ...
        opts = GeminiAgentOptions(
            ...
            # OAuth token) injected by Antigravity. The SDK builds the child
            # env as {**os.environ, **opts.env}, so the inner gemini inherits
            ...
            # descriptors to the top-level gemini process; the SDK-spawned
            ...
            # makes the inner CLI fall back to ~/.gemini/.credentials.json.
    ...
    if filter_mode == "self_refute":
        ...
        refute_prompt = (
            ...
            "(.gemini/settings, .vscode/, package.json scripts — repo "
