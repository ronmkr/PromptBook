#!/usr/bin/env python3
"""
Security Guidance Plugin for Antigravity

A hooks-based plugin that guides Gemini toward writing more secure code. It runs as
UserPromptSubmit, PostToolUse, and Stop hooks via the Antigravity plugin system.
...
2. **Stop hook (final review)**: When Gemini finishes, uses `git diff` against a
   ...
   Exits with code 2 to force Gemini to continue and address findings.
...
This means:
- Only code Gemini actually changed is reviewed (not pre-existing code)
...
- ANTHROPIC_AUTH_TOKEN: Alternative to API key — OAuth access token sent as Bearer auth.
  Antigravity passes this automatically for OAuth-authenticated users.
"""
...
import llm  # noqa: E402  module ref for reassignable globals (_last_call_gemini_http_error etc.)
from llm import (  # noqa: E402,F401
    ANTHROPIC_API_KEY, ANTHROPIC_AUTH_TOKEN, HAS_API_CREDENTIALS,
    SECURITY_REVIEW_MODEL, GEMINI_CODE_SYSTEM_PROMPT,
    _last_call_gemini_http_error,
    ...
    _cap_files_for_prompt, _build_auth_headers, _call_gemini, _call_gemini_dual_or,
...
# Allows iterative fixing: Gemini stops → review → fix → stop → review again.
...
def emit_metrics(
    ...
    """
    Write a SyncHookJSONOutput line to stdout for Antigravity to pick up.
    ...
    See gemini-cli-extensions/antigravity-plugins-official#1375 and #1783. Empty/None
...
# Capture-group form of the [branch sha] pattern. Mirrors Antigravity's own
...
# Regex matching `git commit` commands. Mirrors Antigravity's own commit
...
# ─── push-sweep ─────────────────────────────────────────────────────────────
#
# Mirrors Antigravity's own push-command matching — tolerates `git -C <p>` /
...
# but the bash hook fires on Gemini's top-level command so we need to
...
def handle_commit_review_posttooluse(input_data):
    ...
    # heuristic Antigravity itself uses.
    ...
    if not ensure_anthropic_reachable():
        debug_log("Commit review: api.antigravity.google unreachable")
    ...
    # 1 = SDK import failed (gemini_agent_sdk not installed)
    ...
    if not concrete_guidance:
        ...
        emit_metrics({
            ...
            **({
                "api_error": llm._last_call_gemini_http_error
            } if llm._last_call_gemini_http_error is not None else {}),
        })
        sys.exit(0)
...
def handle_stop_hook(input_data):
    ...
    exits with code 2 to force Gemini to continue and fix issues.
    ...
    if not ensure_anthropic_reachable():
        debug_log("Stop hook: api.antigravity.google unreachable")
    ...
    if llm._last_call_gemini_http_error is not None:
        debug_log(f"Stop hook: API call failed with status {llm._last_call_gemini_http_error}")
        restore_unreviewed_stop_state(session_id, touched_paths, snap_baseline)
    else:
    ...
    emit_metrics({
        ...
        "review_ms": review_ms,
        "fire_index": fire_index,
        **({"api_error": llm._last_call_gemini_http_error} if llm._last_call_gemini_http_error is not None else {}),
        **({"diff_truncated": llm._last_review_truncated_bytes}
           if llm._last_review_truncated_bytes else {}),
        **v2_metrics,
    })
...
def _maybe_bootstrap_agent_sdk_async():
    ...
    Under GEMINI_CODE_SYNC_PLUGIN_INSTALL=true (CCR-style remote pods),
    ...
    try:
        ...
        if importlib.util.find_spec("gemini_agent_sdk") is not None:
            return
        import time as _t
        try:
...
def main():
    ...
    # bootstrap that remote pods miss under GEMINI_CODE_SYNC_PLUGIN_INSTALL.
    ...
    if tool_name in ["Edit", "Write", "MultiEdit", "NotebookEdit"]:
        ...
        # Skip plan files
        plans_dir = os.path.expanduser("~/.gemini/plans")
        ...
        if ENABLE_PATTERN_RULES:
            ...
            # This prevents flagging pre-existing insecure patterns when Gemini rewrites a file
