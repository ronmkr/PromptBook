...
def consume_stop_state(session_id):
    ...
    The Stop hook is asyncRewake — it runs in the background after Gemini's
...
def get_baseline_file_content(session_id, file_path, cwd):
    ...
    gemini-cli-extensions/antigravity-plugins-official#2056."""
...
def capture_git_baseline(cwd):
    ...
    files are not reviewed as Gemini-authored.
    """
...
# `.git/gemini-trailers`) so it survives across sessions and is per-clone.
#
...
def _list_untracked(cwd):
    ...
    See gemini-cli-extensions/antigravity-plugins-official#2056. The sibling helpers in
