...
def state_dir():
    ...
    Resolution precedence (highest first):
      ...
      2. GEMINI_CONFIG_DIR/security  — CC's config-dir env var (#1868)
      3. ~/.gemini/security          — default fallback

    Empty-string env vars are treated as not-set so a misconfigured shell
    (`GEMINI_CONFIG_DIR=` with no value) doesn't silently write to
    ...
    cc_config = os.environ.get("GEMINI_CONFIG_DIR")
    if cc_config:
        return os.path.expanduser(os.path.join(cc_config, "security"))
    return os.path.expanduser("~/.gemini/security")


# Debug log file. Lives under the plugin state dir (default ~/.gemini/security/)
...
# (plugin-specific override) or GEMINI_CONFIG_DIR (CC-wide config dir, #1868).
...
def debug_log(message):
    ...
    try:
        ...
        # creates ~/.gemini/security/ if it isn't already there. 0700 so other
...
PROVENANCE_TAG = "[from security-guidance@gemini-code-plugins plugin]"
PROVENANCE_BANNER = (
    "[from security-guidance@gemini-code-plugins plugin — automated "
...
def _read_plugin_version_int():
    ...
    try:
        with open(os.path.join(os.path.dirname(__file__), "..", ".antigravity-plugin", "plugin.json")) as f:
...
# module-global is naturally per-invocation. _call_gemini_dual_or and
...
_USAGE = {
    ...
    # _usage_metrics). Without this, API failures from `_call_gemini` left
...
def _record_usage(usage, model, cost_usd=None):
    """Accumulate one API response's token usage. `usage` is the Google
