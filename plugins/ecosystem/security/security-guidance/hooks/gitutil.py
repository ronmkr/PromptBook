...
def _diff_pathspec(cwd, paths):
    ...
    git diff. Paths outside cwd (e.g. ~/.gemini/…) are dropped. Returns the
...
def _git_reflog_recent_commits(repo_root, max_age_s=120, max_n=5):
    ...
    try:
        ...
        # missed here. See gemini-cli-extensions/antigravity-plugins-official#2056.
        r = subprocess.run(
...
def get_git_diff(cwd, baseline_sha, full_context=False, paths=None, untracked_paths=None):
    ...
    if paths and not pathspec:
        ...
        # (e.g. only ~/.gemini/... edits). Returning "" flows to skip(6); an
