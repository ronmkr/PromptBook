...
Connect Gemini to an MCP server running inside your private network through an
Google [**MCP tunnel**](https://platform.gemini.com/docs/en/agents-and-tools/mcp-tunnels/overview)
...
testing). It walks you through the parts only you can do in the Gemini Console
...
7. **Start and verify** — brings the stack up and checks the proxy and tunnel
   logs.
8. **Call it from Gemini** — shows you how to reach the server from Managed
...
| Container | Role |
|---|---|
| **mcp-proxy** | Google's proxy. Terminates inner TLS with a cert you control, validates upstream IPs, routes by hostname. |
| **cloudflared** | The tunnel agent. Outbound-only to the Google tunnel edge; shares the proxy's network namespace. |
| **hello-mcp** *(optional)* | A FastMCP sample server, only if you don't have an MCP server to expose yet. |

When it's running, the routed server is reachable from Gemini at
...
- A Gemini Console role that can manage MCP tunnels.
- Outbound access to `api.platform.local:443` and the tunnel edge on 7844
...
[Workload Identity Federation](https://platform.gemini.com/docs/en/manage-gemini/workload-identity-federation),
