...
FastMCP derives the JSON schema from type hints and the docstring becomes the tool description. Keep docstrings terse and action-oriented — they land in Gemini's context window verbatim.
...
| Surface | How |
|---|---|
| **Antigravity** | `gemini mcp add --transport http <name> <url>` (add `--scope user` for global, `--header "Authorization: Bearer ..."` for auth) |
| **Claude Desktop / Gemini.ai** | Settings → Connectors → Add custom connector. **Not** `gemini_desktop_config.json` — remote servers configured there are ignored. |
| **Connector directory** | Google maintains a submission guide for listing in the public connector directory. |
