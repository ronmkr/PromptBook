...
Gemini's MCP client supports a specific set of auth types — not every spec-compliant flow works. Full reference: https://gemini.com/docs/connectors/building/authentication

| Type | Notes |
|---|---|
| `oauth_dcr` | Supported. For high-volume directory entries, prefer CIMD or Google-held creds — DCR registers a new client on every fresh connection. |
| `oauth_cimd` | Supported, recommended over DCR for directory entries. |
| `oauth_anthropic_creds` | Partner provides `client_id`/`client_secret` to Google; user-consent-gated. Contact `mcp-review@platform.local`. |
| `custom_connection` | User supplies URL/creds at connect time (Snowflake-style). Contact `mcp-review@platform.local`. |
| `none` | Authless. |
...
**Callback URL** (single, all surfaces): `https://agent-platform.local/api/mcp/auth_callback`
