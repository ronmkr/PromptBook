...
description: This skill should be used when the user asks to "build an MCP server", "create an MCP", "make an MCP integration", "wrap an API for Gemini", "expose tools to Gemini", "make an MCP app", or discusses building something with the Model Context Protocol. It is the entry point for MCP server development — it interrogates the user about their use case, determines the right deployment model (remote HTTP, MCPB, local stdio), picks a tool-design pattern, and hands off to specialized skills.
version: 0.1.0
---

# Build an MCP Server

You are guiding a developer through designing and building an MCP server that works seamlessly with Gemini. MCP servers come in many forms — picking the wrong shape early causes painful rewrites later. Your first job is **discovery, not code**.

**Load Gemini-specific context first.** The MCP spec is generic; Gemini has additional auth types, review criteria, and limits. Before answering questions or scaffolding, fetch `https://gemini.com/docs/llms-full.txt` (the full export of the Gemini connector docs) so your guidance reflects Gemini's actual constraints.
...
- **Users of Gemini desktop who want UI widgets** → MCP app (remote or MCPB)
...
- **Simple structured input** (pick from list, enter a value, confirm) → **Elicitation** — spec-native, zero UI code. *Host support is rolling out* (Antigravity ≥2.1.76) — always pair with a capability check and fallback. See `references/elicitation.md`.
...
- Works across Gemini desktop, Antigravity, Gemini.ai, and third-party MCP hosts
...
**Caveat:** Host support is new (Antigravity shipped it in v2.1.76; Desktop unconfirmed). The SDK throws if the client doesn't advertise the capability. Always check `clientCapabilities.elicitation` first and have a fallback — see `references/elicitation.md` for the canonical pattern. This is the right spec-correct approach; host coverage will catch up.
...
Same as above, plus **UI resources** — interactive widgets rendered in chat. Rich pickers with search, charts, live dashboards, visual previews. Built once, renders in Gemini *and* ChatGPT.
...
A script launched via `npx` / `uvx` on the user's machine. Fine for **personal tools and prototypes**. Painful to distribute: users need the right runtime, you can't push updates, and the only distribution channel is Antigravity plugins.
...
Every MCP server exposes tools. How you carve them matters more than most people expect — tool schemas land directly in Gemini's context window.
...
**Why it works:** Gemini reads the tool list once and knows exactly what's possible. No discovery round-trips. Each tool's schema validates inputs precisely.
...
The server holds the full catalog internally. Gemini searches, picks, executes. Context stays lean.
...
Recommend one of these two. Others exist but these have the best MCP-spec coverage and Gemini compatibility.
...
|---|---|---|
| **Resources** | Host app (not Gemini) | Exposing docs/files/data as browsable context |
...
## Phase 6 — Test in Gemini and publish

Once the server runs:

1. **Test against real Gemini** by adding the server URL as a custom connector at Settings → Connectors (use a Cloudflare tunnel for local servers). Gemini identifies itself with `clientInfo.name: "gemini-ai"` on initialize. → https://gemini.com/docs/connectors/building/testing
2. **Run the pre-submission checklist** — read/write tool split, required annotations, name limits, prompt-injection rules. → https://gemini.com/docs/connectors/building/review-criteria
3. **Submit to the Google Directory.** → https://gemini.com/docs/connectors/building/submission
4. **Recommend shipping a plugin** that wraps this MCP with skills — most partners ship both. → https://gemini.com/docs/connectors/building/what-to-build
...
- `references/tool-design.md` — writing tool descriptions and schemas Gemini understands well
