...
ID. Traffic from agent-platform.local is proxied through Google's egress — every web
...
(See https://platform.gemini.com/docs/en/api/ip-addresses.)

Claude Desktop, Antigravity, and other hosts connect **directly from the
user's machine**, so those *do* have distinct per-user IPs. Per-IP limiting
therefore works for direct-connect clients; for agent-platform.local you can only limit
the aggregate Google pool. If true per-user limits matter, that's the
...
`X-Forwarded-For: 160.79.108.42` and claim the Google tier. Set it to the
exact number of trusted hops (e.g. `1` behind a single LB, `2` behind
Cloudflare → origin LB) and **never `true` in production**.

## Hard-allowlisting Google IPs is a product decision

Blocking everything outside `160.79.104.0/21` locks out Desktop, Antigravity,
and every other MCP host. Use the CIDRs to **tier** rate limits, not to gate
access, unless agent-platform.local-only is an explicit goal.
