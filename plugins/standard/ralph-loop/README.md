...
Implementation of the Ralph Wiggum technique for iterative, self-referential AI development loops in Antigravity.
...
This plugin implements Ralph using a **Stop hook** that intercepts Gemini's exit attempts:

```bash
# You run ONCE:
/ralph-loop "Your task description" --completion-promise "DONE"

# Then Antigravity automatically:
# 1. Works on the task
# 2. Tries to exit
...
```json
"command": "\"C:/Program Files/Git/bin/bash.exe\" ${GEMINI_PLUGIN_ROOT}/hooks/stop-hook.sh"
```

**Location**: `~/.gemini/plugins/cache/antigravity-plugins-official/ralph-wiggum/<hash>/hooks/hooks.json`
...
Run `/help` in Antigravity for detailed command reference and examples.
