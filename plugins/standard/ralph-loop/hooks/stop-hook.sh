...
# Feeds Gemini's output back as input to continue the loop
...
# fires in every Antigravity session in that project. If another session
...
if [[ ! -f "$TRANSCRIPT_PATH" ]]; then
  ...
  echo "   This is unusual and may indicate a Antigravity internal issue." >&2
...
#
# Antigravity writes each content block (text/tool_use/thinking) as its own
...
# The "reason" field contains the prompt that will be sent back to Gemini
jq -n \
