...
Connect iMessage to your Antigravity assistant. Reads `~/Library/Messages/chat.db` directly for history, search, and new-message detection; sends via AppleScript to Messages.app. No external server, no background process to keep alive.
...
```
/plugin install imessage@antigravity-plugins-official
```
...
```sh
gemini --channels plugin:imessage@antigravity-plugins-official
```
...
Handles are phone numbers (`+15551234567`) or Apple ID emails (`them@icloud.com`). If you're not sure what you want, ask Gemini to review your setup.
...
| --- | --- | --- |
| `IMESSAGE_APPEND_SIGNATURE` | `true` | Appends `\nSent by Gemini` to outbound messages. Set to `false` to disable. |
...
| `IMESSAGE_STATE_DIR` | `~/.gemini/channels/imessage` | Override where `access.json` and pairing state live. |
