...
`~/.gemini/channels/discord/access.json`. You never talk to Discord — you
...
`~/.gemini/channels/discord/access.json`:
...
1. Read `~/.gemini/channels/discord/access.json` (handle missing file).
2. Show: dmPolicy, allowFrom count and list, pending count with codes +
   sender IDs + age, groups count.

### `pair <code>`

1. Read `~/.gemini/channels/discord/access.json`.
...
7. `mkdir -p ~/.gemini/channels/discord/approved` then write
   `~/.gemini/channels/discord/approved/<senderId>` with `chatId` as the
