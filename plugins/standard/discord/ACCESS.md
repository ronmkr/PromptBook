...
| Config file | `~/.gemini/channels/discord/access.json` |
...
```
/discord:access set mentionPatterns '["^hey gemini\\b", "\\bassistant\\b"]'
```
...
`~/.gemini/channels/discord/access.json`. Absent file is equivalent to `pairing` policy with empty lists, so the first DM triggers pairing.
