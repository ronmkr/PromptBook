...
Connect a Discord bot to your Antigravity with an MCP server.

When the bot receives a message, the MCP server forwards it to Gemini and provides tools to reply, react, and edit messages.
...
These are Antigravity commands — run `gemini` to start a session first.

Install the plugin:
```
/plugin install discord@antigravity-plugins-official
/reload-plugins
```
...
Writes `DISCORD_BOT_TOKEN=...` to `~/.gemini/channels/discord/.env`. You can also write that file by hand, or set the variable in your shell environment — shell takes precedence.
...
```sh
gemini --channels plugin:discord@antigravity-plugins-official
```

**7. Pair.**

With Antigravity running from the previous step, DM your bot on Discord — it replies with a pairing code. If the bot doesn't respond, make sure your session is running with `--channels`. In your Antigravity session:
...
Pairing is for capturing IDs. Once you're in, switch to `allowlist` so strangers don't get pairing-code replies. Ask Gemini to do it, or `/discord:access policy allowlist` directly.
...
| `download_attachment` | Download all attachments from a specific message by ID to `~/.gemini/channels/discord/inbox/`. Returns file paths + metadata. Use when `fetch_messages` shows a message has attachments. |
...
Downloads land in `~/.gemini/channels/discord/inbox/`.
