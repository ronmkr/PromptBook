...
external service. Open a browser, type, messages go to your Antigravity
...
Install the plugin:
```
/plugin install fakechat@antigravity-plugins-official
```

**Relaunch with the channel flag** — the server won't connect without this. Exit your session and start a new one:

```sh
gemini --channels plugin:fakechat@antigravity-plugins-official
```
...
Inbound images/files save to `~/.gemini/channels/fakechat/inbox/` and the path
