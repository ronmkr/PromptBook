...
1. Use Glob tool to find all hookify rule files:
   ```
   pattern: ".gemini/hookify.*.local.md"
   ```
...
**Status**: ✅ Active
**File**: .gemini/hookify.dangerous-rm.local.md
```
...
2. Or manually create `.gemini/hookify.my-rule.local.md` files
...
Check `${GEMINI_PLUGIN_ROOT}/examples/` for example rule files.
```
