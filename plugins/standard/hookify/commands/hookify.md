...
For each confirmed behavior, create a `.gemini/hookify.{rule-name}.local.md` file:
...
{Message to show Gemini when rule triggers}
```
...
**IMPORTANT**: Rule files must be created in the current working directory's `.gemini/` folder, NOT the plugin directory.

Use the current working directory (where Antigravity was started) as the base path.

1. Check if `.gemini/` directory exists in current working directory
   - If not, create it first with: `mkdir -p .gemini`

2. Use Write tool to create each `.gemini/hookify.{name}.local.md` file
   - Use relative path from current working directory: `.gemini/hookify.{name}.local.md`
   - The path should resolve to the project's .gemini directory, not the plugin's

3. Show user what was created:
   ```
   Created 3 hookify rules:
   - .gemini/hookify.dangerous-rm.local.md
   - .gemini/hookify.console-log.local.md
   - .gemini/hookify.sensitive-files.local.md
...
4. Create `.gemini/hookify.dangerous-rm.local.md`:
   ```markdown
   ---
...
- **File location**: Create files in project's `.gemini/` directory (current working directory), NOT the plugin's .gemini/
...
2. Ensure `.gemini/` directory exists (create with mkdir if needed)
3. Use absolute path if needed: `{cwd}/.gemini/hookify.{name}.local.md`
4. Verify file was created with Glob or ls

**If rule doesn't trigger after creation:**
1. Verify file is in project `.gemini/` not plugin `.gemini/`
