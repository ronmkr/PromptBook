...
This analyzes your request and creates `.gemini/hookify.warn-rm.local.md`.

### 2. Test It Immediately

**No restart needed!** Rules take effect on the very next tool use.

Ask Gemini to run a command that should trigger the rule:
```
Run rm -rf /tmp/test
```
...
- **`stop`**: Triggers when Gemini wants to stop (for completion checks)
...
**This rule blocks the operation** - Gemini will not be allowed to execute these commands.
...
**This rule warns but allows** - Gemini sees the message but can still proceed.
...
**This blocks Gemini from stopping** if no test commands appear in the session transcript. Enable only when you want strict enforcement.
...
```bash
rm .gemini/hookify.my-rule.local.md
```
...
This plugin is part of the Antigravity Marketplace. It should be auto-discovered when the marketplace is installed.
...
**Rule not triggering:**
1. Check rule file exists in `.gemini/` directory (in project root, not plugin directory)
