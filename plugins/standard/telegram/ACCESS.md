...
| Config file | `~/.gemini/channels/telegram/access.json` |
...
```
/telegram:access set mentionPatterns '["^hey gemini\\b", "\\bassistant\\b"]'
```
...
`~/.gemini/channels/telegram/access.json`. Absent file is equivalent to `pairing` policy with empty lists, so the first DM triggers pairing.
