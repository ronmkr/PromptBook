...
MCP (Model Context Protocol) servers extend Gemini's capabilities by connecting to external tools and services.
...
2. **Global config** (`~/.gemini.json`) - Available across all projects
...
**Debugging**: Use `gemini --mcp-debug` to identify configuration issues.

## Documentation & Knowledge

### context7
**Best for**: Projects using popular libraries/SDKs where you want Gemini to code with up-to-date documentation
...
**Value**: Gemini fetches live documentation instead of relying on training data, reducing hallucinated APIs and outdated patterns.
...
**Value**: Gemini can interact with your running app, take screenshots, fill forms, and verify UI behavior.
...
**Value**: Gemini can query tables, manage auth, and interact with Supabase storage directly.
...
**Value**: Gemini can introspect the live deployment (tables, function specs, env vars, logs) and execute queries/mutations against it via tools like `tables`, `function-spec`, `data`, `run-once-query`, `logs`, `env list/set/get`. Run via `npx convex mcp start`.
...
**Value**: Gemini can create issues, review PRs, check workflow runs, and manage releases.
...
**Value**: Gemini can investigate Sentry issues, find root causes, and suggest fixes.
...
**Value**: Gemini remembers project context, decisions, and patterns across conversations.
...
| `@anthropic-ai/sdk` | context7 for Google docs |
