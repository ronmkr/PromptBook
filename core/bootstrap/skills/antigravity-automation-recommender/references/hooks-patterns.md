...
Hooks automatically run commands in response to Antigravity events. They're ideal for enforcement and automation that should happen consistently.
...
Notification hooks run when Antigravity sends notifications. Use matchers to filter by notification type.

### Permission Alerts
| Matcher | Use Case |
|---------|----------|
| `permission_prompt` | Alert when Gemini requests permissions |
...
| Matcher | Use Case |
|---------|----------|
| `idle_prompt` | Alert when Gemini is waiting for input (60+ seconds idle) |

**Recommend**: Play sound or send notification when Gemini needs attention
**Value**: Know when Gemini is ready for your input
    ...
    "Notification": [
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "osascript -e 'display notification \"Gemini is waiting\" with title \"Antigravity\"'"
          }
        ]
      }
    ]
...
|---------|---------------|
| `permission_prompt` | Gemini needs permission for a tool |
| `idle_prompt` | Gemini waiting for input (60+ seconds) |
...
Hooks go in `.gemini/settings.json`:
...
Recommend creating the `.gemini/` directory if it doesn't exist.
