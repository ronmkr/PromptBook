...
Skills are packaged expertise with workflows, reference materials, and best practices. Create them in `.gemini/skills/<name>/SKILL.md`. Skills can be invoked by Gemini automatically when relevant, or by users directly with `/skill-name`.
...
Create project-specific skills in `.gemini/skills/<name>/SKILL.md`.
...
user-invocable: false           # Only Gemini can invoke (for background knowledge)
...
```
.gemini/skills/create-migration/
├── SKILL.md
└── scripts/
...
3. Run validation: `bash ~/.gemini/skills/create-migration/scripts/validate-migration.sh`
...
### Project Conventions (Gemini-only)

Background knowledge Gemini applies automatically:
...
The command output replaces the placeholder before Gemini sees the skill content.
