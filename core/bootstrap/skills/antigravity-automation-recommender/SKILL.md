...
description: Analyze a codebase and recommend Antigravity automations (hooks, subagents, skills, plugins, MCP servers). Use when user asks for automation recommendations, wants to optimize their Antigravity setup, mentions improving Antigravity workflows, asks how to first set up Antigravity for a project, or wants to know what Antigravity features they should use.
...
Create skills in `.gemini/skills/<name>/SKILL.md`. Some are also available via plugins:
...
| Code style | **project-conventions** | Gemini-only |
...
See [referencesplatform-installs-reference.md](referencesplatform-installs-reference.md) for available plugins.

| Codebase Signal | Recommended Plugin |
|-----------------|-------------------|
| General productivity | **gemini-agent-skills** - Core skills bundle |
...
```markdown
## Antigravity Automation Recommendations
...
**Install**: `gemini mcp add context7`
...
#### [skill name]
**Why**: [specific reason]
**Create**: `.gemini/skills/[name]/SKILL.md`
**Invocation**: User-only / Both / Gemini-only
...
- `user-invocable: false` — Gemini-only (for background knowledge)
...
Recommend headless Gemini for automated pipelines:

```bash
# Pre-commit hook example
gemini -p "fix lint errors in src/" --allowedTools Edit,Write

# CI pipeline with structured output
gemini -p "<prompt>" --output-format stream-json | your_command
```

### Permissions for Hooks

Configure allowed tools in `.gemini/settings.json`:
