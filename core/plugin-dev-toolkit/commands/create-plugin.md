...
3. Create directory structure using bash:
   ```bash
   mkdir -p plugin-name/.antigravity-plugin
...
6. Create .gitignore if needed (for .gemini/\*.local.md, etc.)
...
2. For each skill:
   ...
   - For user-invoked skills (slash commands): include `description`, `argument-hint`, and `allowed-tools` frontmatter; write instructions FOR Gemini (not TO user)
...
2. For each command:
   ...
   - Write instructions FOR Gemini (not TO user)
...
2. Create .mcp.json configuration with:
   ...
   - Command and args (with ${GEMINI_PLUGIN_ROOT})
...
3. Create example .gemini/plugin-name.local.md file (as documentation)
4. Implement settings reading in hooks/commands as needed
5. Add to .gitignore: `.gemini/*.local.md`
...
5. **Test hook configuration** (if plugin has hooks):
   ...
   - Verify ${GEMINI_PLUGIN_ROOT} usage
...
1. **Installation instructions**:
   - Show user how to test locally:
     ...
     ```
   - Or copy to `.antigravity-plugin/` for project testing
...
3. **Testing recommendations**:
   ...
   - For hooks: Use `gemini --debug` to see hook execution
...
- **Apply best practices**:
  ...
  - Skill instructions written FOR Gemini (not TO user)
  - Strong trigger phrases
  - ${GEMINI_PLUGIN_ROOT} for portability
