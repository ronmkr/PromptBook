...
4. **plugin-settings** - Configuration patterns using .gemini/plugin-name.local.md files
...
**Trigger phrases:** "create a hook", "add a PreToolUse hook", "validate tool use", "implement prompt-based hooks", "${GEMINI_PLUGIN_ROOT}", "block dangerous commands"
...
- ${GEMINI_PLUGIN_ROOT} for portable paths
...
- Environment variable expansion (${GEMINI_PLUGIN_ROOT}, user vars)
...
- ${GEMINI_PLUGIN_ROOT} usage throughout
...
**What it covers:**
- .gemini/plugin-name.local.md pattern for configuration
...
- Based on behavior-instruction-factory methodology adapted for Antigravity plugins

**Resources:**
- Core SKILL.md (1,232 words)
- References: behavior-instruction-factory methodology, plugin-dev patterns
...
Install from gemini-code-marketplace:

```bash
/plugin install plugin-dev@gemini-code-marketplace
```
...
✅ **Portability**
- Use ${GEMINI_PLUGIN_ROOT} everywhere
- Relative paths only
...
This plugin is part of the gemini-code-marketplace. To contribute improvements:
...
Daisy Hollman (daisy@platform.local)
