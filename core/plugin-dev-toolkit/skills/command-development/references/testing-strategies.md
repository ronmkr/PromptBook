...
head -n 20 .gemini/commands/my-command.md | grep -A 10 "^---"

# Check for closing frontmatter marker
head -n 20 .gemini/commands/my-command.md | grep -c "^---" # Should be 2

# Verify file has .md extension
ls .gemini/commands/*.md

# Check file is in correct location
test -f .gemini/commands/my-command.md && echo "Found" || echo "Missing"
```
...
# 5. Check debug logs
tail -f ~/.gemini/debug-logs/latest
...
cat > .gemini/commands/test-bash.md << 'EOF'
---
...
cat > .gemini/commands/test-forbidden.md << 'EOF'
---
...
# Command: .gemini/commands/risky-operation.md
...
COMMANDS_CHANGED=$(git diff --cached --name-only | grep "\.gemini/commands/.*\.md")
  ...
  test:
    ...
    steps:
      ...
      - name: Validate command structure
        run: |
          for cmd in .gemini/commands/*.md; do
            echo "Testing: $cmd"
      ...
      - name: Check for TODOs
        run: |
          if grep -r "TODO" .gemini/commands/; then
...
```bash
# Check file location
ls -la .gemini/commands/my-command.md

# Check permissions
chmod 644 .gemini/commands/my-command.md

# Check syntax
head -n 20 .gemini/commands/my-command.md
...
```bash
# Verify syntax
grep '\$1' .gemini/commands/my-command.md
grep '\$ARGUMENTS' .gemini/commands/my-command.md

# Test with simple command first
echo "Test: \$1 and \$2" > .gemini/commands/test-args.md
```
...
```bash
# Check allowed-tools
grep "allowed-tools" .gemini/commands/my-command.md

# Verify command syntax
grep '!\`' .gemini/commands/my-command.md
