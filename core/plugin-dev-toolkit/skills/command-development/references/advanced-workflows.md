...
Deployment state saved to `.gemini/deployment-state.local.md`:
...
Reading deployment state from `.gemini/deployment-state.local.md`...
...
```markdown
.gemini/plugin-name-workflow.local.md:
...
Reading workflow state from .gemini/plugin-name-workflow.local.md...

Current stage: @.gemini/plugin-name-workflow.local.md
...
State file: @.gemini/plugin-name-workflow.local.md
...
Creating: .gemini/feature-complete.flag
...
if [ -f .gemini/feature-complete.flag ]; then
...
if [ -f .gemini/deployment.lock ]; then
...
!`validate.sh`
echo "checkpoint:validation" >> .gemini/deployment-checkpoints.log

## Checkpoint 2: Build
!`build.sh`
echo "checkpoint:build" >> .gemini/deployment-checkpoints.log

## Checkpoint 3: Deploy
!`deploy.sh`
echo "checkpoint:deploy" >> .gemini/deployment-checkpoints.log
...
Written to .gemini/deployment-state.local.md
...
Reading state: @.gemini/deployment-state.local.md
...
rm .gemini/deployment-state.local.md
