...
!`node ${GEMINI_PLUGIN_ROOT}/scripts/quality-check.js $1`
...
**Key features:**
- Uses `${GEMINI_PLUGIN_ROOT}` for portable path
...
**Security scan:**
!`bash ${GEMINI_PLUGIN_ROOT}/scripts/security-scan.sh $1`

**Performance analysis:**
!`bash ${GEMINI_PLUGIN_ROOT}/scripts/perf-analyze.sh $1`

**Best practices check:**
!`bash ${GEMINI_PLUGIN_ROOT}/scripts/best-practices.sh $1`
...
Template structure: @${GEMINI_PLUGIN_ROOT}/templates/api-documentation.md
...
!`bash ${GEMINI_PLUGIN_ROOT}/scripts/pre-release-check.sh $1`

**Step 2 - Build artifacts:**
!`bash ${GEMINI_PLUGIN_ROOT}/scripts/build-release.sh $1`

**Step 3 - Run test suite:**
!`bash ${GEMINI_PLUGIN_ROOT}/scripts/run-tests.sh`

**Step 4 - Package release:**
!`bash ${GEMINI_PLUGIN_ROOT}/scripts/package.sh $1`
...
Deployment configuration for $1: @${GEMINI_PLUGIN_ROOT}/config/$1-deploy.json
...
- Plugin's linting rules: ${GEMINI_PLUGIN_ROOT}/config/lint-rules.json
- Security checklist: ${GEMINI_PLUGIN_ROOT}/checklists/security.md
- Performance guidelines: ${GEMINI_PLUGIN_ROOT}/docs/performance.md
...
Run plugin analyzer: !`node ${GEMINI_PLUGIN_ROOT}/scripts/analyze.js $1`
...
Check build script exists: !`test -x ${GEMINI_PLUGIN_ROOT}/scripts/build.sh && echo "EXISTS" || echo "MISSING"`

Verify configuration available: !`test -f ${GEMINI_PLUGIN_ROOT}/config/$1.json && echo "FOUND" || echo "NOT_FOUND"`

If all validations pass:

**Configuration:** @${GEMINI_PLUGIN_ROOT}/config/$1.json

**Execute build:** !`bash ${GEMINI_PLUGIN_ROOT}/scripts/build.sh $1 2>&1`

**Validation results:** !`bash ${GEMINI_PLUGIN_ROOT}/scripts/validate-build.sh $1 2>&1`
...
Load environment configuration: @${GEMINI_PLUGIN_ROOT}/config/$1-checks.json

Determine check level: !`echo "$1" | grep -E "^prod$" && echo "FULL" || echo "BASIC"`

**For production environment:**
- Full test suite: !`bash ${GEMINI_PLUGIN_ROOT}/scripts/test-full.sh`
- Security scan: !`bash ${GEMINI_PLUGIN_ROOT}/scripts/security-scan.sh`
- Performance audit: !`bash ${GEMINI_PLUGIN_ROOT}/scripts/perf-check.sh`
- Compliance check: !`bash ${GEMINI_PLUGIN_ROOT}/scripts/compliance.sh`

**For non-production environments:**
- Basic tests: !`bash ${GEMINI_PLUGIN_ROOT}/scripts/test-basic.sh`
- Quick lint: !`bash ${GEMINI_PLUGIN_ROOT}/scripts/lint.sh`
...
```markdown
Check exists: !`test -f ${GEMINI_PLUGIN_ROOT}/path/file && echo "YES" || echo "NO"`
```
...
2. **Verify ${GEMINI_PLUGIN_ROOT} expansion:**
   ```bash
   # Add debug output to command
   !`echo "Plugin root: ${GEMINI_PLUGIN_ROOT}"`
   ```
...
4. **Validate resource availability:**
   ...
   !`ls -la ${GEMINI_PLUGIN_ROOT}/scripts/`
   !`ls -la ${GEMINI_PLUGIN_ROOT}/config/`
   ```

### Common Mistakes to Avoid

1. **Using relative paths instead of ${GEMINI_PLUGIN_ROOT}:**
   ```markdown
   # Wrong
   !`node ./scripts/analyze.js`

   # Correct
   !`node ${GEMINI_PLUGIN_ROOT}/scripts/analyze.js`
   ```
...
2. **Forgetting to allow required tools:**
   ...
   allowed-tools: Bash(*)
   ---
   !`bash ${GEMINI_PLUGIN_ROOT}/scripts/script.sh`
   ```
...
4. **Hardcoding plugin paths:**
   ...
   @${GEMINI_PLUGIN_ROOT}/config.json
   ```
