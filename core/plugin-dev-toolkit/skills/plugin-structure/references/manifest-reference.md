...
**Required path**: `.antigravity-plugin/plugin.json`

The manifest MUST be in the `.antigravity-plugin/` directory at the plugin root. Antigravity will not recognize plugins without this file in the correct location.
...
```json
{
  "mcpServers": {
    "github": {
      "command": "node",
      "args": ["${GEMINI_PLUGIN_ROOT}/servers/github-mcp.js"],
      "env": {
