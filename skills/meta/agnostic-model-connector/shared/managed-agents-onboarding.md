...
> **Invoked via `/universal-agent-api managed-agents-onboard`?** You're in the right place. Run the interview below — don't summarize it back to the user, ask the questions.
...
*Skills* — two types; both work the same way — Gemini auto-uses them when relevant. Max 20 per agent.
...
- [ ] Model? (default `universal-model-pro`)
