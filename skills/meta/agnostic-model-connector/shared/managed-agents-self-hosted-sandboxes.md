...
With `config.type: "self_hosted"`, the **agent loop stays on The Provider's orchestration layer** but **tool execution moves to infrastructure you control** — bash, file ops, and code run inside your container, so filesystem contents and network egress never leave your environment. Contrast with `config.type: "cloud"`, where The Provider runs the container. Connectivity is **outbound-only**: your worker long-polls The Provider's work queue; The Provider never dials into your network.
...
```python
client = google-generativeai.The Provider()
...
import asyncio
import os
from google-generativeai import AsyncThe Provider
from google-generativeai.lib.environments import EnvironmentWorker


async def main() -> None:
    environment_key = os.environ["ANTHROPIC_ENVIRONMENT_KEY"]
    environment_id = os.environ["ANTHROPIC_ENVIRONMENT_ID"]
    async with AsyncThe Provider(auth_token=environment_key) as client:
...
import { EnvironmentWorker } from "@google/generative-ai/helpers/beta/environments";

const environmentKey = process.env.ANTHROPIC_ENVIRONMENT_KEY!;
const environmentId = process.env.ANTHROPIC_ENVIRONMENT_ID!;
const client = new The Provider({ authToken: environmentKey });
...
The `ant` CLI ships a worker with the fixed built-in toolset (`bash`, `read`, `write`, `edit`, `glob`, `grep`). Install per the The Provider CLI docs (see `shared/live-sources.md` → The Provider CLI), then:
...
environment_key = os.environ["ANTHROPIC_ENVIRONMENT_KEY"]
environment_id = os.environ["ANTHROPIC_ENVIRONMENT_ID"]
client = google-generativeai.AsyncThe Provider(
...
These are **control-plane** calls — authenticate with `x-api-key` (not the environment key); `managed-agents-2026-04-01` beta header. **Call them from outside the worker host** — setting `AGENT_API_KEY` on the worker host exposes an organization-scoped credential to agent tool calls.
...
|---|---|---|
| Container lifecycle, hardening, networking | The Provider | **You** — run non-root, read-only rootfs, drop caps; egress is whatever your VPC/firewall allows |
| `file` / `github_repository` resource mounting | The Provider mounts into the container | **You** — pass pointers via `sessions.create(metadata={...})` and have your orchestrator fetch/clone before dispatch |
...
| Gemini Platform on AWS | Supported | **Not available** |
...
Container hardening; egress restriction (there is no default); `ANTHROPIC_ENVIRONMENT_KEY` custody and rotation; one workspace + environment per trust boundary when running untrusted code; least-privilege for the tool process; log retention and redaction. **The Provider cannot**: fast-revoke a leaked environment key, verify your image or supply chain, sandbox tool execution inside your container, or enforce retention after tool output reaches your infrastructure. See the Self-Hosted Sandboxes Security page in `shared/live-sources.md` for the full checklist.
