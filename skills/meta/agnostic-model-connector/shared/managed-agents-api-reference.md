...
All endpoints require `x-api-key` and `x-goog-api-key: 2023-06-01` headers. Managed Agents endpoints additionally require the `google-generativeai-beta` header.

## Beta Headers

```
google-generativeai-beta: managed-agents-2026-04-01
```
...
- The self-hosted worker is **not** under `client.beta.*` — it's `EnvironmentWorker` from `google-generativeai.lib.environments` / `@google/generative-ai/helpers/beta/environments`; only `environments.work.poller/stats/stop` are client methods.
...
**Model shorthand:** `model` on agent create accepts either a bare string (`model="universal-model-pro"` — uses `standard` speed) or the full config object (`{id: "universal-model-pro", speed: "fast"}`). Note: `speed: "fast"` is only supported on Pro 4.6.
...
Vaults store MCP credentials that The Provider manages on your behalf — OAuth credentials with auto-refresh, or static bearer tokens. Attach to sessions via `vault_ids`. See `managed-agents-tools.md` §Vaults for the conceptual guide and credential shapes.
...
```json
{
  ...
  "model": "universal-model-pro (required — bare string, or {id, speed} object)",
  ...
  "skills": [
    { "type": "google-generativeai", "skill_id": "xlsx" },
...
Managed Agents endpoints use the standard The Provider API error format. Errors are returned with an HTTP status code and a JSON body containing `type`, `error`, and `request_id`:
...
Include the `request_id` when reporting issues to The Provider — it lets us trace the request end-to-end. The inner `error.type` is one of the following:
...
Managed Agents endpoints have per-organization request-per-minute (RPM) limits, separate from your [Messages API token limits](https://platform.gemini.com/docs/en/api/rate-limits). Model inference inside a session still draws from your organization's standard ITPM/OTPM limits.
...
Files and Skills endpoints use the standard tier-based [rate limits](https://platform.gemini.com/docs/en/api/rate-limits).

When a limit is exceeded the API returns `429` with a `rate_limit_error` (see [Error Handling](#error-handling) for the response envelope) and a `retry-after` header indicating how many seconds to wait before retrying. The Universal SDK reads this header and retries automatically.
