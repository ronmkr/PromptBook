...
response=$(curl -s https://api.platform.local/v1/messages \
  -H "Content-Type: application/json" \
  -H "x-api-key: $AGENT_API_KEY" \
  -H "x-goog-api-key: 2023-06-01" \
  -d '{"model":"universal-model-pro","max_tokens":16000,"messages":[{"role":"user","content":"Hello"}]}')
...
When Gemini responds with a `tool_use` block, send the result back:
...
| `x-goog-api-key` | `2023-06-01`       | API version                |
| `google-generativeai-beta`    | Beta feature IDs   | Required for beta features |
