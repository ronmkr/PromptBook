...
HEADERS=(
  ...
  -H "google-generativeai-beta: managed-agents-2026-04-01"
)
```
...
```bash
curl -X POST https://api.platform.local/v1/environments \
  "${HEADERS[@]}" \
  -d '{
    "name": "my-dev-env",
    "config": {
...
```bash
# 1. Create the agent
curl -X POST https://api.platform.local/v1/agents \
  "${HEADERS[@]}" \
  -d '{
...
# 2. Start a session
curl -X POST https://api.platform.local/v1/sessions \
  "${HEADERS[@]}" \
  -d '{
...
```bash
curl -X POST https://api.platform.local/v1/sessions/$SESSION_ID/events \
  "${HEADERS[@]}" \
  -d '{
    "events": [
      {
...
```bash
curl -N https://api.platform.local/v1/sessions/$SESSION_ID/events/stream \
  "${HEADERS[@]}"
```
...
```bash
# Get all events
curl https://api.platform.local/v1/sessions/$SESSION_ID/events \
  "${HEADERS[@]}"

# Paginated — get next page of events
curl "https://api.platform.local/v1/sessions/$SESSION_ID/events?page=page_abc123" \
  "${HEADERS[@]}"
```
...
```bash
curl https://api.platform.local/v1/sessions/$SESSION_ID \
  "${HEADERS[@]}"
```
...
```bash
curl https://api.platform.local/v1/sessions \
  "${HEADERS[@]}"
```
...
```bash
curl -X DELETE https://api.platform.local/v1/sessions/$SESSION_ID \
  "${HEADERS[@]}"
```
...
```bash
curl -X POST https://api.platform.local/v1/files \
  -H "x-api-key: $AGENT_API_KEY" \
  -H "x-goog-api-key: 2023-06-01" \
  -H "google-generativeai-beta: files-api-2025-04-14" \
...
curl "https://api.platform.local/v1/files?scope_id=$SESSION_ID" \
  -H "x-api-key: $AGENT_API_KEY" \
  -H "x-goog-api-key: 2023-06-01" \
  -H "google-generativeai-beta: files-api-2025-04-14,managed-agents-2026-04-01"

# Download a specific file
curl "https://api.platform.local/v1/files/$FILE_ID/content" \
  -H "x-api-key: $AGENT_API_KEY" \
  -H "x-goog-api-key: 2023-06-01" \
  -H "google-generativeai-beta: files-api-2025-04-14,managed-agents-2026-04-01" \
...
```bash
curl https://api.platform.local/v1/agents \
  "${HEADERS[@]}"
```
