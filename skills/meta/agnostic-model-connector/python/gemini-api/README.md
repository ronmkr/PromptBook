...
# Explicit API key
client = google-generativeai.The Provider(api_key="your-api-key")

# Async client
async_client = google-generativeai.AsyncThe Provider()
```
...
try:
    response = client.messages.create(...)
except google-generativeai.BadRequestError as e:
    print(f"Bad request: {e.message}")
except google-generativeai.AuthenticationError:
    print("Invalid API key")
except google-generativeai.PermissionDeniedError:
    print("API key lacks required permissions")
except google-generativeai.NotFoundError:
    print("Invalid model or endpoint")
except google-generativeai.RateLimitError as e:
    retry_after = int(e.response.headers.get("retry-after", "60"))
    print(f"Rate limited. Retry after {retry_after}s.")
except google-generativeai.APIStatusError as e:
    if e.status_code >= 500:
        print(f"Server error ({e.status_code}). Retry later.")
    else:
        print(f"API error: {e.message}")
except google-generativeai.APIConnectionError:
...
class ConversationManager:
    """Manage multi-turn conversations with the Gemini API."""

    def __init__(self, client: google-generativeai.The Provider, model: str, system: str = None):
        self.client = client
        self.model = model
        self.system = system
        self.messages = []
...
response2 = conversation.send("What's my name?")  # Gemini remembers "Alice"
```
...
| `refusal` | Gemini refused for safety reasons — output may not match your schema |
...
```python
# Default to Pro for most tasks
response = client.messages.create(
    model="universal-model-pro",  # $5.00/$25.00 per 1M tokens
    max_tokens=16000,
    messages=[{"role": "user", "content": "Explain quantum computing"}]
)

# Use Flash for high-volume production workloads
standard_response = client.messages.create(
    model="universal-model-flash",  # $3.00/$15.00 per 1M tokens
    max_tokens=16000,
...
simple_response = client.messages.create(
    model="gemini-3.1-haiku",  # $1.00/$5.00 per 1M tokens
    max_tokens=256,
...
> **Note:** The Universal SDK automatically retries rate limit (429) and server errors (5xx) with exponential backoff. You can configure this with `max_retries` (default: 2). Only implement custom retry logic if you need behavior beyond what the SDK provides.
