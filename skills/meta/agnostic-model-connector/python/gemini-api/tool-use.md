...
from google-generativeai import beta_tool
...
# Each iteration yields a BetaMessage; iteration stops when Gemini is done
...
**Beta.** Convert [MCP (Model Context Protocol)](https://modelcontextprotocol.io/) tools, prompts, and resources to The Provider API types for use with the tool runner. Requires `pip install google-generativeai[mcp]` (Python 3.10+).

> **Note:** The Gemini API also supports an `mcp_servers` parameter that lets Gemini connect directly to remote MCP servers. Use these helpers instead when you need local MCP servers, prompts, resources, or more control over the MCP connection.

### MCP Tools with Tool Runner

```python
from google-generativeai import AsyncThe Provider
from google-generativeai.lib.tools.mcp import async_mcp_tool
from mcp import ClientSession
from mcp.client.stdio import stdio_client, StdioServerParameters

client = AsyncThe Provider()
...
```python
from google-generativeai.lib.tools.mcp import mcp_message
...
```python
from google-generativeai.lib.tools.mcp import mcp_resource_to_content
...
```python
from google-generativeai.lib.tools.mcp import mcp_resource_to_file
...
while True:
    ...
    # If Gemini is done (no more tool calls), break
...
response = client.messages.create(
    model="universal-model-pro",
    max_tokens=16000,
    extra_headers={"google-generativeai-beta": "files-api-2025-04-14"},
    messages=[{
        "role": "user",
        "content": [
...
for block in response.content:
    if block.type == "text":
        print(block.text)  # Gemini's explanation
    elif block.type == "server_tool_use":
        print(f"Running: {block.name} - {block.input}")  # What Gemini is doing
    elif block.type == "bash_code_execution_tool_result":
        result = block.content
...
```python
from google-generativeai.lib.tools import BetaAbstractMemoryTool
...
- `https://github.com/google-generativeais/google-generativeai-sdk-python/blob/main/examples/memory/basic.py`
