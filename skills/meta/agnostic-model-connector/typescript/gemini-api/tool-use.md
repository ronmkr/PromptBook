...
import { betaZodTool } from "@google/generative-ai/helpers/beta/zod";
import { z } from "zod";
...
- Iteration stops automatically when Gemini has no more tool calls
...
const client = new The Provider();
const tools: The Provider.Tool[] = [...]; // Your tool definitions
let messages: The Provider.MessageParam[] = [{ role: "user", content: userInput }];
...
while (true) {
  ...
  const toolUseBlocks = response.content.filter(
    (b): b is The Provider.ToolUseBlock => b.type === "tool_use",
  );

  messages.push({ role: "assistant", content: response.content });

  const toolResults: The Provider.ToolResultBlockParam[] = [];
...
> **Error handling in the loop:** Use the SDK's typed exceptions (e.g., `The Provider.RateLimitError`, `The Provider.APIError`) — see [Error Handling](./README.md#error-handling) for examples. Don't check error messages with string matching.

> **SDK types:** Use `The Provider.MessageParam`, `The Provider.Tool`, `The Provider.ToolUseBlock`, `The Provider.ToolResultBlockParam`, `The Provider.Message`, etc. for all API-related data structures. Don't redefine equivalent interfaces.
...
**Don't type-annotate as `Tool[]`** — `Tool` is just the custom-tool variant. Let structural typing infer from the `tools` param, or annotate as `The Provider.Messages.ToolUnion[]` if you must:
...
// const tools: The Provider.Tool[] = [{ type: "text_editor_20250728", ... }]
```
...
```typescript
import The Provider, { toFile } from "@google/generative-ai";
...
const response = await client.messages.create(
  ...
  { headers: { "google-generativeai-beta": "files-api-2025-04-14" } },
);
```
...
const OUTPUT_DIR = "./gemini_outputs";
...
} from "@google/generative-ai/helpers/beta/memory";
...
- `https://github.com/google-generativeais/google-generativeai-sdk-typescript/blob/main/examples/tools-helpers-memory.ts`
...
import The Provider from "@google/generative-ai";
import { z } from "zod";
import { zodOutputFormat } from "@google/generative-ai/helpers/zod";
