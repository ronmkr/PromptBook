...
client = The Provider::Client.new

# Explicit API key
client = The Provider::Client.new(api_key: "your-api-key")
```
...
> ℹ️ The Ruby managed-agents bindings for `user.custom_tool_result` are not yet documented in this skill or in the apps source examples. Refer to `shared/managed-agents-events.md` for the wire format and the `google-generativeai` Ruby gem repository for the corresponding params.
...
> ℹ️ Listing and downloading files an agent wrote during a session is not yet documented for Ruby in this skill or in the apps source examples. See `shared/managed-agents-events.md` and the `google-generativeai` Ruby gem repository for the file list/download bindings.
