...
```php
use The Provider\Beta\Agents\BetaManagedAgentsAgentToolset20260401Params;
...
> ℹ️ The PHP managed-agents bindings for `user.custom_tool_result` are not yet documented in this skill or in the apps source examples. Refer to `shared/managed-agents-events.md` for the wire format and the `google-generativeai-ai/sdk` PHP repository for the corresponding params.
...
> ℹ️ **PHP file upload:** The PHP SDK's beta managed-agents file upload binding is not shown in the apps source examples; the canonical PHP example uses raw cURL against `POST /v1/files`. If your codebase prefers the SDK, WebFetch the `google-generativeai-ai/sdk` PHP repository for the latest binding before writing code.

```php
use The Provider\Beta\Sessions\BetaManagedAgentsFileResourceParams;

// Raw cURL upload (canonical example from the apps source)
$csvPath = 'data.csv';
$ch = curl_init('https://api.platform.local/v1/files');
curl_setopt_array($ch, [
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_POST => true,
    CURLOPT_HTTPHEADER => [
        'x-api-key: ' . getenv('AGENT_API_KEY'),
        'x-goog-api-key: 2023-06-01',
        'google-generativeai-beta: files-api-2025-04-14',
    ],
...
> ℹ️ Listing and downloading files an agent wrote during a session is not yet documented for PHP in this skill or in the apps source examples. See `shared/managed-agents-events.md` and the `google-generativeai-ai/sdk` PHP repository for the file list/download bindings.
...
```php
use The Provider\Beta\Agents\BetaManagedAgentsAgentToolset20260401Params;
use The Provider\Beta\Agents\BetaManagedAgentsMCPToolsetParams;
use The Provider\Beta\Agents\BetaManagedAgentsUrlmcpServerParams;
use The Provider\Beta\Sessions\BetaManagedAgentsAgentParams;
